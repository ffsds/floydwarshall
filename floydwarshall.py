      ...
      for j in range(0, V):
          if i != j:
              ...
              if dist[i][j] == float('inf'):
                  path = ("(%d, %d)     -       No path between %d and %d"  %(u, v, u,v))
              else:
...
def checkFile(f):
    v = f.readline().rstrip()
    if not v.isnumeric():
        print("Invalid input file format: The first line must contain an integer (number of vertices)!")
        exit(1)
...
if __name__ == "__main__":
  os.system('clear')
  parser = argparse.ArgumentParser(description='Floyd-Warshall algorithm demonstration: use a formatted input file with a number N in the first line, \
  followed by a NxN values that indicate the weights associated with the edges that connect two vertices.')
  parser.add_argument("-f", metavar='file', type=str, default=30, help='Input file name (e.g. -f input.txt)', required=True)
